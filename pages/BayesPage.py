...
def predict(X, class_probs, conditional_probs, target_col):
    ...
    for cls in class_probs:
        ...
        for feature, value in X.items():
            ...
            st.write(f"P({feature} = {value} | {target_col} = {cls}) = {cond_prob:.3f}")
            prob *= cond_prob
        results[cls] = prob
        st.write(f"--> P(X|{cls}) * P({cls}) = {results[cls]:.5f}")
...
def main(file_path, test_inputs):
    ...
    for i, row in test_inputs.iterrows():
        X_test = row.to_dict()
        st.write(f"\n========== Test Case {i+1} ==========")
        st.write(f"Test Input: {X_test}")

        # Tính xác suất
        class_probs = calculate_class_probs(df, target_col)
        st.write("\nP(Ci):")
        for cls, prob in class_probs.items():
            st.write(f"P({target_col} = {cls}) = {prob:.3f}")
